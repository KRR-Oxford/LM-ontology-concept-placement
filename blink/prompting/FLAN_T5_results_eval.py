...
# bug to be fixed (so far manually post-processed) - negative False at ins rate@1_all; and dot after number in the answer to be removed
...
def extract_pred(pred_raw):
    ...
    '''    
    pred_raw = pred_raw[6:-4] # get core answer
...
for data_split_mark in data_splits:
    result_filename = "../../models/biencoder/mm+%s2017AA-tl-sapbert-NIL-bs16/top%d_candidates/%s%s-top%d-preds%s-prompts-by-edges%s.csv" % (snomed_subset,top_k_base_value,data_split_mark,'-NIL' if is_NIL else '',top_k_value, "-degree-1" if filter_by_degree else "",model_name_clean)
