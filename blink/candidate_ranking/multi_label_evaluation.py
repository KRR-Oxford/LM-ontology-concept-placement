...
def all_metrics(yhat, y, k=8, yhat_raw=None, calc_auc=True, is_multi_hot_complete=None):
    """
        Inputs:
    ...
    #AUC and @k (prec_at, rec_at, f1_at, acc_at_all, acc_at_any)
    if yhat_raw is not None and calc_auc:
        ...
        for k_i in k:
            ...
            ins_at_k_all = insertion_at_k_all(yhat_raw, y, k_i,is_multi_hot_complete=is_multi_hot_complete)
            metrics['ins_at_%d_all' % k_i] = ins_at_k_all
            ins_at_k_any = insertion_at_k_any(yhat_raw, y, k_i)
            metrics['ins_at_%d_any' % k_i] = ins_at_k_any            

        roc_auc = auc_metrics(yhat_raw, y, ymic)
        metrics.update(roc_auc)
    
    #insertion rate metrics: acc_all, acc_any
    metrics["ins_all"] = insertion_all(yhat,y,is_multi_hot_complete=is_multi_hot_complete)
    metrics["ins_any"] = insertion_any(yhat,y)
...
#########################################################################
#INSERTION METRICS: calculate metric on acc-any and acc-all (adapted by Hang Dong)
#it is an instance-level group of metrics, 
# but focus on any of the label being matched (acc-any) and 
#              all of the label being matched (acc-all)
#########################################################################

def insertion_all(yhat, y, is_multi_hot_complete=None):
    #print('left part:', (intersect_size(yhat, y, 1) == union_size(yhat, y, 1)) 
    #           & (intersect_size(yhat, y, 1) != 0))
    #print('right part, is_multi_hot_complete:',is_multi_hot_complete)
    num = np.sum((intersect_size(yhat, y, 1) == union_size(yhat, y, 1)) 
               & (intersect_size(yhat, y, 1) != 0) & is_multi_hot_complete)
    num_ave = num / yhat.shape[0]
    print('num in insertion_all:',num, 'num_ave:', num_ave)
    return num_ave

def insertion_any(yhat, y):
    num = np.sum(intersect_size(yhat, y, 1) > 0)
    #print('intersect_size(yhat, y, 1):', intersect_size(yhat, y, 1))
    num_ave = num / yhat.shape[0]
    print('num in insertion_any:',num, 'num_ave:', num_ave)
    return num_ave

def insertion_at_k_all(yhat_raw, y, k, is_multi_hot_complete=None):
    #num true labels in top k predictions / num true labels
    sortd = np.argsort(yhat_raw)[:,::-1]
    topk = sortd[:,:k] # get mat: rows * the pred topk indices

    #get recall at k all for each example
    vals = []
    for i, tk in enumerate(topk): # loop over rows, each contains the pred topk indices
        row_is_multi_hot_complete = is_multi_hot_complete[i]
        #print('row_is_multi_hot_complete:',row_is_multi_hot_complete)
        num_true_in_top_k = y[i,tk].sum() # sum of all true elements in the topk pred in y
        denom = y[i,:].sum() # sum of all true elements in y
        if row_is_multi_hot_complete and (num_true_in_top_k == denom) and (denom != 0): # if the multi-hot rep is complete (has all true recorded) AND all in top_k is true
            vals.append(1)
        else:
            vals.append(0)
...
def insertion_at_k_any(yhat_raw, y, k):
    #num true labels in top k predictions / num true labels
    sortd = np.argsort(yhat_raw)[:,::-1]
    topk = sortd[:,:k]

    #get recall at k any for each example
    vals = []
    for i, tk in enumerate(topk):
        num_true_in_top_k = y[i,tk].sum() # sum of all true elements in the topk pred in y
        #denom = y[i,:].sum() # sum of all true elements in y
        if num_true_in_top_k > 0: # if any in top_k is true
            vals.append(1)
        else:
            vals.append(0)
